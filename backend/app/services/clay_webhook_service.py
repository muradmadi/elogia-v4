...
class ClayWebhookService:
    ...
    async def send_webhook_notification(
        ...
        """
        Send a notification webhook to Clay about job status.
        
        Args:
            job_id: Unique identifier for the enrichment job
            status: Current status of the job
            message: Optional message about the job status
            
        Returns:
            Dict containing response from Clay
        """
        callback_url = f"{self.public_base_url}/webhooks/clay?job_id={job_id}"
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code >= 400:
                    raise HTTPException(
                        status_code=status.HTTP_502_BAD_GATEWAY,
                        detail=f"Clay notification failed with status {response.status_code}",
                    )
            ...
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail=f"Failed to send notification to Clay: {str(e)}",
            )
        except Exception as e:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Unexpected error sending notification: {str(e)}",
            )
