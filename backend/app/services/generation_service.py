...
class GenerationService:
    ...
    This service creates its own database session to ensure it can operate
    independently when run as a background task (the request-scoped session
    would be closed after the HTTP response).
    """
        ...
        async with AsyncSessionLocal() as db_session:
            try:
                ...
                if sequence is None:
                    ...
                    sequence.status = CampaignSequenceStatus.GENERATING
                ...
                try:
                    ...
                    validated_sequence = validate_claude_response(raw_response)
                    ...
                    sequence.status = CampaignSequenceStatus.FAILED
                    await db_session.commit()
                    raise
                ...
                sequence.status = CampaignSequenceStatus.COMPLETED
                await db_session.commit()
                await db_session.refresh(sequence)
